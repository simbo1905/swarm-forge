          ...
          (if (or (str/blank? line) (str/starts-with? line "#"))
            ...
            (let [fields (str/split line #"\s+")]
              ...
              (let [[keyword role agent worktree & trailing] fields
                ...
                (when-not (#{"claude" "codex" "copilot" "grok" "opencode" "vibe"} agent)
  ...
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  ...
                  "vibe" (str "vibe --workdir " (sq (str role-worktree)) " --trust --agent accept-edits --output streaming " (extra-args-prefix row) "-p \"$(cat " (sq (str prompt-file)) ")\"")))
      (= index 0)
